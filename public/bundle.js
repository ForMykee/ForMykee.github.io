...
// Similar a la función 'PalabraPotencia', pero esta función genera la palabra de manera inversa.
// Similar a la función 'PalabraPotencia', pero esta función genera la palabra de manera inversa.
const PalabraPotenciaNeg = (indice, longitud) => {
  ...
  if (longitud === 0) {
    ...
    const resto = indice % alfabeto.length;
    const letra = alfabeto[alfabeto.length - 1 - resto]; // Selecciona la letra desde el final del alfabeto
    const nuevoIndice = Math.floor(indice / alfabeto.length);
    // En esta función, la letra se agrega antes de llamar a la función recursiva,
    // lo que resulta en una palabra generada en orden inverso.
    palabra += letra;
...
BtnPotencia.addEventListener("click", (evento) => {
  ...
  if (resultado !== null) {
    ...
    resultadoPotencia.textContent = "La potencia es: " + resultado.join(", ");
  }
});
...
BtnPlacas.addEventListener("click", (evento) => {
  ...
  if (placas !== null) {
    resultadoPlaca.textContent = "La placa " + placas + " es valida";
  }
});
...
BtnPlacasEdo.addEventListener("click", (evento) => {
  ...
  if (placas !== null) {
    resultadoPlacaEdoMex.textContent = "La placa " + placas + " es valida";
  }
});
...
BtnPlacasMor.addEventListener("click", (evento) => {
  ...
  if (placas !== null) {
    resultadoPlacaMorelos.textContent = "La placa " + placas + " es valida";
  }
});
